...
class RebootBladeViewerPlugin(HostViewerPlugin):
    def __init__(self):
        HostViewerPlugin.__init__(self)

    def name(self):
        return QString("iLo - Reboot blade")

    def icon(self):
        return QString(":/images/host_reboot")

    def view(self, hostList):
        cmd = ["/drd/software/int/sys/renderops/rpower", "reboot"]
        hosts = []
...
if currentUser.userGroups().size() > 0:
    ...
    for userGroup in groupList:
        if userGroup.group().name() == "Admin" or userGroup.group().name() == "RenderOps":
            HostViewerFactory.registerPlugin( RebootBladeViewerPlugin() )
            break
