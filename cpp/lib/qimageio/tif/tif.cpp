...
bool TIFHandler::canRead() const
{
    return canRead(device());
}

bool TIFHandler::read(QImage *outImage)
{
    TIFF *tiff;
    uint32 width, height;
    uint32 *data;
...
bool TIFHandler::write(const QImage &)
{
    // TODO: stub
    return false;
}

QByteArray TIFHandler::name() const
{
    return "tiff";
}

bool TIFHandler::canRead(QIODevice *device)
{
    if (!device) {
        qWarning("TIFHandler::canRead() called with no device");
        return false;
    }
...
class TIFPlugin : public QImageIOPlugin
{
public:
...
QStringList TIFPlugin::keys() const
{
    return QStringList() << "tiff" << "TIFF" << "tif" << "TIF";
}

QImageIOPlugin::Capabilities TIFPlugin::capabilities(QIODevice *device, const QByteArray &format) const
{
    ...
    Capabilities cap;
    if (device->isReadable() && TIFHandler::canRead(device))
        cap |= CanRead;
    return cap;
}

QImageIOHandler *TIFPlugin::create(QIODevice *device, const QByteArray &format) const
{
    QImageIOHandler *handler = new TIFHandler;
    handler->setDevice(device);
    handler->setFormat(format);
    return handler;
}

Q_EXPORT_STATIC_PLUGIN(TIFPlugin)
Q_EXPORT_PLUGIN2(tiff, TIFPlugin)
