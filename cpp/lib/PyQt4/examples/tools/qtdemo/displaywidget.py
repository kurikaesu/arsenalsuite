...
class DisplayWidget(QtGui.QWidget):
    ...
    actionRequested = QtCore.pyqtSignal(str)

    categoryRequested = QtCore.pyqtSignal(str)

    displayEmpty = QtCore.pyqtSignal()

    documentationRequested = QtCore.pyqtSignal(str)

    exampleRequested = QtCore.pyqtSignal(str)

    launchRequested = QtCore.pyqtSignal(str)
        ...
        for shape in self.shapes:
            if shape.rect().contains(QtCore.QPointF(event.pos())) and "fade" not in shape.metadata:
                if "action" in shape.metadata:
                    self.actionRequested.emit(shape.metadata["action"])
                elif "category" in shape.metadata:
                    self.categoryRequested.emit(shape.metadata["category"])
                elif "example" in shape.metadata:
                    self.exampleRequested.emit(shape.metadata["example"])
                elif "documentation" in shape.metadata:
                    self.documentationRequested.emit(shape.metadata["documentation"])
                    shape.metadata["highlight"] = False
                    self.enableUpdates()
                elif "launch" in shape.metadata:
                    self.launchRequested.emit(shape.metadata["launch"])
    ...
    def timerEvent(self, event):
        if event.timerId() == self.timer.timerId():
            ...
            super(DisplayWidget, self).timerEvent(event)
