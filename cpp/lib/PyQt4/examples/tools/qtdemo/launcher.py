...
from displayshape import TitleShape, DisplayShape, PanelShape, ImageShape, \
        DocumentShape

class Launcher(QtGui.QMainWindow):

    showPage = QtCore.pyqtSignal()
    ...
    def __init__(self, parent=None):
        ...
        parentPageAction1.triggered.connect(self.showPage)
        parentPageAction2.triggered.connect(self.showPage)
        parentPageAction3.triggered.connect(self.showPage)
        ...
        self.resizeTimer.timeout.connect(self.redisplayWindow)

        self.display.actionRequested.connect(self.executeAction)
        self.display.categoryRequested.connect(self.showExamples)
        self.display.documentationRequested.connect(self.showExampleDocumentation)
        self.display.exampleRequested.connect(self.showExampleSummary)
        self.display.launchRequested.connect(self.launchExample)

        self.showPage.connect(self.showParentPage,
                type=QtCore.Qt.QueuedConnection)
        self.windowResized.connect(self.redisplayWindow,
                type=QtCore.Qt.QueuedConnection)
    ...
    def executeAction(self, action):
        if action == "parent":
            ...
            if len(self.runningExamples) == 0:
                self.display.displayEmpty.connect(self.close)
                self.display.reset()
            else:
                self.close()
    ...
    def launchExample(self, uniquename):
        ...
        process.finished.connect(self.enableLaunching)
    ...
    def showExampleDocumentation(self, uniqueName):
        # The connections may not already exist.
        try:
            self.display.displayEmpty.disconnect(self.resizeWindow)
            self.display.displayEmpty.disconnect(self.close)
        except:
            pass
    ...
    def showParentPage(self):
        ...
        # The connection may not already exist.
        try:
            self.slideshowTimer.timeout.disconnect(self.updateExampleSummary)
        except:
            pass
    ...
    def closeEvent(self, event):
        ...
        for example in list(self.runningProcesses.keys()):
            example.terminate()
    ...
    def toggleFullScreen(self):
        ...
        self.displaydisplayEmpty.connect(self.resizeWindow,
                type=QtCore.Qt.QueuedConnection)
        self.display.reset()
